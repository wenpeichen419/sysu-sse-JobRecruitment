<template>
    <div class="job-center-page">
      ...
      <!-- 列表 -->
      <div class="job-list">
        <div v-for="item in paged" :key="item.id" class="job-item" @click="toDetail(item.id)">
          <div class="job-logo">
          ...
          <div class="job-right-info">
            <div class="company-name"><span class="link">{{ item.company }}</span></div>
            <div class="company-details">
              <span class="department">{{ item.department }}</span>
              <span class="divider">|</span>
              <span>投递时间 {{ item.appliedAt }}</span>
            </div>
          </div>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      setPage(p) { if (p>=1 && p<=this.totalPages) { this.page = p; window.scrollTo({top:0,behavior:'smooth'}) } },
      toDetail(id) { this.$router.push({ name: 'AppliedDetail', params: { id } }) },
      statusText(s) {
        return s==='submitted' ? '已投递'
          : s==='interview' ? '已发面试'
          : s==='passed'    ? '已通过'
          : s==='stopped'   ? '流程中止'
          : s
