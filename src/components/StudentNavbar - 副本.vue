...
<script>
// ✅ 导入API方法获取学生信息
import { getWelcomeInfo } from '@/api'
...
export default {
  ...
  methods: {
    ...
    async loadStudentName() {
      try {
        const data = await getWelcomeInfo()
        this.studentName = data.full_name
        console.log('【导航栏加载学生名字成功】', this.studentName)
      } catch (error) {
        console.error('【导航栏加载学生名字失败】', error)
        // 如果加载失败，显示默认文本
        this.studentName = '学生'
      }
    },
